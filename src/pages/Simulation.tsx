    ...
    <div className="flex h-screen">
      <div className="flex flex-1 flex-col">
        <div className="flex items-center justify-between border-b border-border px-6 py-4 glass">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold">{t(`scenario.${scenario.id}.title` as any)}</h1>
        ...
        <div ref={chatRef} className="flex-1 overflow-y-auto px-6 py-6 space-y-4">
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.sender === "user" ? "justify-end" : "justify-start"} animate-fade-in`}>
              ...
              <div className={`max-w-[70%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-soft ${
        ...
        <div className="flex items-center gap-3 border-t border-border px-6 py-4 glass">
          ...
          <Input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder={t("sim.typeResponse")} className="h-12 rounded-2xl bg-card border-border" />
      ...
      <div className="hidden lg:flex w-72 flex-col gap-6 border-l border-border bg-card p-6">
        <div>
          <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">{t("sim.aiEmotion")}</h3>
        ...
        <div>
          <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">{t("sim.yourEmpathy")}</h3>
          <div className="mt-3 flex items-center justify-between text-sm">
        ...
        <div className="mt-auto rounded-2xl bg-muted/50 p-4">
          <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{t("sim.goal")}</p>
          <p className="mt-1 font-semibold text-foreground">{(location.state as any)?.goal || t("setup.showEmpathy")}</p>
        </div>
      </div>
    </div>
  );
