...
const hintToSlider = (h: string) => (h === "low" ? 20 : h === "high" ? 80 : 50);
const sliderToHint = (v: number) => (v < 40 ? "low" : v > 70 ? "high" : "normal");
...
const Settings = () => {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      profilesApi.getMe().then((p) => {
        setVoiceInput(p.voice_input_enabled);
        setHintFrequency([hintToSlider(p.hint_frequency || "normal")]);
        setLoaded(true);
  ...
  const saveVoiceInput = (v: boolean) => {
    setVoiceInput(v);
    if (isAuthenticated) profilesApi.updateMe({ voice_input_enabled: v }).catch(() => {});
  };

  const saveHintFrequency = (v: number[]) => {
    setHintFrequency(v);
    if (isAuthenticated) profilesApi.updateMe({ hint_frequency: sliderToHint(v[0]) }).catch(() => {});
  };
  ...
  return (
    <div className="page-container max-w-2xl">
      ...
      <Card className="mt-4 shadow-soft border-border overflow-hidden">
        <CardContent className="divide-y divide-border p-0">
          <div className="flex items-center justify-between px-5 py-4">
            ...
            <Switch checked={voiceInput} onCheckedChange={saveVoiceInput} />
          </div>
          <div className="flex items-center justify-between px-5 py-4">
            <div className="flex items-center gap-4">
          ...
          {showHints && (
            <div className="px-5 py-5">
              ...
              <Slider value={hintFrequency} onValueChange={saveHintFrequency} max={100} step={1} />
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="mt-4 shadow-soft border-border overflow-hidden">
        <CardContent className="divide-y divide-border p-0">
          <button
            className="flex w-full items-center justify-between px-5 py-4 tap-scale hover:bg-muted/40 transition-colors"
